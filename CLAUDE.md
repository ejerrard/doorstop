...
- **`cli.py`** — Typer app wiring the pipelines together: `discover-ministerial-diaries` (`cabinet.py` -> `load.py`), `download-ministerial-diaries` (`load.py`'s `still_listed_diary_urls` -> `download.py` -> `load.py`'s `pdf_diff`/`merge_pdf_snapshots`), and `extract` (`extract.py` -> `load.py` for a directory of PDFs).
